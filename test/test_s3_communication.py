...
from city_details_proccesing import Stats, create_city_stats_from_city_data, combine_stats, City
from configuration import AGGREGATED_STATS_FILE_NAME
from conftest import generate_example_city_data, EXAMPLE_ID_1, generate_example_cities, EXAMPLE_COUNTRY_1, \
    EXAMPLE_COUNTRY_2, EXAMPLE_ID_2
from s3_communication import get_s3_client, create_bucket, push_city_stats_to_s3, \
...
async def test_data_pushed_to_s3_with_stats(run_dummy_moto):
    ...
    async with get_s3_client() as s3_client:
        ...
        assert Stats.create_stats_from_s3_metadata(metadata["Metadata"]) == create_city_stats_from_city_data(example_city_data)
...
async def test_create_aggregated_stats_for_country_and_date(run_dummy_moto):
    ...
    example_city_data_on_desired_date = generate_example_city_data(desired_date)[EXAMPLE_ID_1]
    example_city_data_on_irrelevant_date = generate_example_city_data(irrelevant_date)[EXAMPLE_ID_2]

    raw_city_stats_on_desired_date = json.dumps(example_city_data_on_desired_date).encode("utf-8")
    raw_city_stats_on_irrelevant_date = json.dumps(example_city_data_on_irrelevant_date).encode("utf-8")
    ...
    async with get_s3_client() as s3_client:
        ...
        push_data_from_desired_country_desired_date_tasks = [
            push_city_stats_to_s3(relevant_city_1, desired_date, raw_city_stats_on_desired_date, s3_client),
            push_city_stats_to_s3(relevant_city_2, desired_date, raw_city_stats_on_desired_date, s3_client)
            ]

        push_data_from_desired_country_irrelevant_date_task = push_city_stats_to_s3(relevant_city_1, irrelevant_date, raw_city_stats_on_irrelevant_date, s3_client)
        push_data_from_irrelevant_country_desired_date_task = push_city_stats_to_s3(city_in_irrelevant_country, desired_date,
                                                                                    raw_city_stats_on_desired_date,
                                                                                    s3_client)
        await asyncio.gather(*push_data_from_desired_country_desired_date_tasks,
                             push_data_from_desired_country_irrelevant_date_task,
                             push_data_from_irrelevant_country_desired_date_task)

        expected_aggregated_results = combine_stats([create_city_stats_from_city_data(example_city_data_on_desired_date)]*2)

        # Act
        await create_aggregated_stats_for_country_and_date(desired_country, desired_date, s3_client)

        # Assert
        metadata_of_aggregated_stats_file = (await s3_client.head_object(Bucket=desired_country, Key=f"{desired_date}/{AGGREGATED_STATS_FILE_NAME}"))["Metadata"]
        assert Stats.create_stats_from_s3_metadata(metadata_of_aggregated_stats_file) == expected_aggregated_results
