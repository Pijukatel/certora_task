...
from litestar import Litestar, post, get, Request, MediaType
import aiohttp
from types_aiobotocore_s3 import S3Client

from ref_server_communication import get_cities, get_raw_city_stats_from_ref_server, \
    parse_start_and_end_date_from_query_params, expected_date_format
...
async def collect_cities_data_to_s3(date: str) -> None:
    checked_date = datetime.datetime.strptime(date,
                                              expected_date_format).date()
    async with get_s3_client() as s3_client:
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector()) as session:
            ...
            transfer_city_stats_tasks = (
                asyncio.create_task(_transfer_city_stats_to_s3(session, s3_client, city, checked_date))
                for
                city in cities)
...
async def get_country_stats(request: Request) -> Result:
    ...
    dates = [str(start_date + datetime.timedelta(days=days)) for days in range((end_date - start_date).days + 1)]

    async with get_s3_client() as s3_client:
        countries = [bucket["Name"] for bucket in (await s3_client.list_buckets())["Buckets"]]

        aggregate_stats_tasks = []
        for country in countries:
            for date in dates:
                aggregate_stats_tasks.append(
                    asyncio.create_task(get_aggregated_stats_for_country_and_date(country, date, s3_client)))
