...
Schema is available at http://127.0.0.1:8000/schema (or http://127.0.0.1:8000/schema/swager).
"""
...
async def get_city_stats(city_id: int, date: date) -> list[dict[str, Any]]:
    await asyncio.sleep(random() * 4.2)
    dt = datetime.combine(date, datetime.min.time())
    return [
        {
            "departure-time": dt + timedelta(minutes=randint(0, 720)),
